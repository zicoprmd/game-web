...
function Pong() {
  ...
  useEffect(() => {
    ...
    const update = (timestamp) => {
      ...
      // CPU AI - simple tracking with deltaTime
      ...
      if (Math.abs(diff) > 5) {
        let newCpuY
        if (diff > 0) {
          newCpuY = Math.min(CANVAS_HEIGHT - PADDLE_HEIGHT, cpuYRef.current + cpuSpeed * deltaTime)
        } else {
          newCpuY = Math.max(0, cpuYRef.current - cpuSpeed * deltaTime)
        }
        setCpuY(newCpuY)
